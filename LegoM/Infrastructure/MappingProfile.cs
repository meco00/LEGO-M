  ...
  using AutoMapper;
    ...
    public class MappingProfile:Profile
    {
        ...
        public MappingProfile()
        {
            ...
            .ForMember(p => p.MainImageUrl, pd => pd.MapFrom(x => x.Images.Where(x => x.isDeleted == false).Select(x => x.ImageUrl).FirstOrDefault()))
            .ForMember(p => p.SecondImageUrl, pd => pd.MapFrom(x => x.Images.Where(x => x.isDeleted == false).Skip(1).Take(1).Select(x => x.ImageUrl).FirstOrDefault()))
            .ForMember(p => p.ThirdImageUrl, pd => pd.MapFrom(x => x.Images.Where(x => x.isDeleted == false).Skip(2).Take(1).Select(x => x.ImageUrl).FirstOrDefault()));
